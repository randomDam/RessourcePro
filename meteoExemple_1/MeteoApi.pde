...
class Meteo {
	...
	float sys_sunrise = 0; //heure du levé de soleil (timeStamp)
	float sys_sunset = 0; //heure du couhé de soleil (timeStamp)
	...
	void autoFillVar(){
		...
		try{
			sys_sunrise = rawData.getJSONObject("sys").getFloat("sunrise");
		}catch(Exception e){}
		try{
			sys_sunset = rawData.getJSONObject("sys").getFloat("sunset");
		}catch(Exception e){}
